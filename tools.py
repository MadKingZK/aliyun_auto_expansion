...
class AliEcsTools(object):
    def __init__(self, ali_key, ali_secret, region_id):
        ...
        self.client = AcsClient(self.ali_key, self.ali_secret,)
    ...
    def get_last_snap_id(self, disk_id):
        ...
        for snaps in self.find_last_snapshot(disk_id):
            for snap in snaps:
                ...
                ts = int(
                    time.mktime(
                        time.strptime(snap.get("CreationTime"), "%Y-%m-%dT%H:%M:%SZ")
                    )
                )
                last_snap_id, ts_cur = (
                    (snap_id, ts) if ts > ts_cur else (last_snap_id, ts_cur)
                )
        return last_snap_id
    ...
    def create_image(self, snap_id):
        ...
        request.set_ImageName(
            "auto_expansion_{date_time}".format(
    ...
    def add_to_slb(self, svid, instance_ids, weight):
        ...
        backend_servers = [
            {"ServerId": instance_id, "Port": "80", "Weight": weight, "Type": "ecs"}
...
class ZbxApiTools(object):
    ...
    def getHosts(self):
        hosts = self.zapi.host.get(
            output="extend", hostids="10321", monitored_hosts=None,
        )
        return hosts
...
class JumpServerClient(object):
    ...
    def create_assets(self, ip, hostname, admin_uid, node_lst):
        ...
        response = requests.post(
            url, headers=self.header_info, data=asset, verify=False
        )
    ...
    def del_assets(self, ip):
        ...
        response = requests.delete(
            url, headers=self.header_info, params=param, verify=False
        )
        print(response.text)
...
# 初始化日志模块
def logger_init():
    logger.add('logs/INFO.log', format="{time} {level} {message}", level="INFO", rotation="500MB",
               compression="zip")
    logger.add('logs/ERROR.log', format="{time} {level} {message}", level="ERROR", rotation="500MB",
               compression="zip")
    logger.add('logs/DEBUG.log', format="{time} {level} {message}", level="DEBUG", rotation="500MB",
               compression="zip")
    return logger
